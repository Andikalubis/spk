<?php
    ...
    class Kriteria extends CI_Controller {
					...
					foreach ($data['kriteria'] as $row2) {
						if ($i < $ii) {
							...
							if ($nilai_input < 1) {
								...
								$nilai_2 = number_format(1 / abs($nilai_input), 4);
							} elseif ($nilai_input > 1) {
								$nilai_1 = number_format(1 / abs($nilai_input), 4);
		...
		public function ahp_get_normalisasi($matrik, $jumlah_kolom)
		{
			...
			for ($i = 0; $i < count($matrik); $i++) {
				for ($ii = 0; $ii < count($matrik); $ii++) {
					$matrik_normalisasi[$i][$ii] = number_format($matrik[$i][$ii] / $jumlah_kolom[$ii], 4);
				}
			}
		...
		public function ahp_get_prioritas($matrik_normalisasi)
		{
			...
			for ($i = 0; $i < count($matrik_normalisasi); $i++) {
				...
				$prioritas[$i] = number_format($prioritas[$i] / count($matrik_normalisasi), 4);
			}
			return $prioritas;
		}
		...
		public function ahp_get_matrik_baris($prioritas, $matrik_kriteria)
		{
			...
			for ($i = 0; $i < count($matrik_kriteria); $i++) {
				for ($ii = 0; $ii < count($matrik_kriteria); $ii++) {
					$matrik_baris[$i][$ii] = number_format($prioritas[$ii] * $matrik_kriteria[$i][$ii], 4);
				}
			}
			return $matrik_baris;
		}
	...
	<tr>
		...
		<td>= ' . number_format($lambda_maks, 4) . '</td>
	</tr>
	<tr>
		<td width="100">CI</td>
